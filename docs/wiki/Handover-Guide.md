...
זהו חשבון המנהלת שנמסר לעמותה. שמרי עותק של הפרטים במקום מוגן:

| פרט | הערך |
|---|---|
| **אימייל** | `app@lissan.org` |
| **סיסמה** | הסיסמה נמסרת ישירות לרכזת העמותה ואינה מופיעה במסמך. |
| **נמסר בתאריך** | 27 / 07 / 2026 |

> **מומלץ להחליף את הסיסמה בכניסה הראשונה** — ראי סעיף 4.2, "שינוי סיסמת
> המנהלת". זהו חשבון המנהלת היחיד במערכת.

במערכת קיימת גם תלמידה אחת לדוגמה — **תלמידה לדוגמה** (`demo@lissan.org`) —
שנוצרה כדי לוודא שהחשבון עובד. גם הסיסמה שלה נמסרת ישירות ואינה מופיעה
במסמך. אפשר למחוק אותה בכל שלב.
...
מומלץ לעשות זאת בכניסה הראשונה, כדי שהסיסמה תהיה ידועה רק לעמותה.

1. לחצי על הלשונית **מורות**.
2. אתרי את הכרטיס של החשבון שלך ולחצי עליו **עריכה**.
3. הקלידי סיסמה חדשה בשדה **סיסמה חדשה** (לפחות 6 תווים).
4. לחצי על **שמירת שינויים**.

<img src="images/21-admin-password.png" width="370" alt="שינוי סיסמת המנהלת">

> שדה הסיסמה ריק תמיד. אם משאירים אותו ריק — הסיסמה הקיימת נשמרת ואינה
> משתנה. באותו אופן משנים סיסמה גם למורה או למנהלת אחרת.
...
**ניקוי מסד הנתונים למסירה:** המסד נוקה במנייה של האוספים החיים
(`db.listCollections()`) ומחיקת כל מה שאינו ברשימת השימור — ולא לפי רשימת
מחיקה שנגזרה מהקוד, משום ששניים משלושת האוספים שנותרו לא היו גלויים לבדיקת
קוד: `vocabProgress` נכתב דרך קבוע ולא דרך מחרוזת, ו-`chatConversations` הוא
סכימה מתה שאינה מופיעה בקוד כלל.
